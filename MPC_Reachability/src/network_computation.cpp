...
void print_vector(
  vector< datatype > buffer_vector
)
{
  int i = 0;
  cout << "Printing the vector : " ;
  while(i < buffer_vector.size())
  {
    cout << buffer_vector[i] << " " ;
    i++;
  }
  cout << endl;
}
...
void read_glucose_and_insulin_from_file(
...
)
{
  ...
  unsigned int no_of_G_values = 37;
  unsigned int no_of_I_values = 37;

  int i;
  G_vector.clear();
  i = 0;
  while(i < no_of_G_values)
  {
    file >> buffer;
    G_vector.push_back(buffer);
    i++;
  }

  I_vector.clear();
  i = 0;
  while(i < no_of_I_values)
  {
    file >> buffer;
    I_vector.push_back(buffer);
    i++;
  }
  ...
  if(!file.eof())
  {
    cout << "Some mistake in reading the input point, since you haven't reached the " <<
    " End of File yet " << endl;
  }

}


void print_pancreas_data(
  vector< vector< datatype > > input_data
)
{
  assert(input_data.size() > 0);
  assert(input_data[0].size() > 0);
  int total_lines = input_data.size();
  int no_of_values = input_data[0].size();

  int i, j;

  char file_name[] = "simulation_data";
  ofstream file;
  file.open(file_name);
