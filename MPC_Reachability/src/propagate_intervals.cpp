...
network_handler :: network_handler(const char* name)
{
  ...
  file >> buffer;
  no_of_inputs = (unsigned int) buffer ;
  file >> buffer;
  no_of_outputs = (unsigned int) buffer ;
  file >> buffer;
  no_of_hidden_layers = (unsigned int) buffer ;
...
datatype network_handler :: get_network_output(
  vector< datatype > input
)
{
    assert(no_of_outputs == 1);
    return compute_network_output(input, actual_weights, actual_biases);
}
...
int compute_control_action(
  network_handler prediction_network,
...
{
  unsigned int no_of_glucose_inputs = glucose_inputs.size();
  unsigned int no_of_insulin_inputs = insulin_inputs.size();

  if(prediction_network.no_of_inputs != (no_of_glucose_inputs + no_of_insulin_inputs))
  {
    cout << "Mismatch in the number of inputs the network expects and the " <<
    " number of inputs avaiable in compute_control_action() " << endl;
  }
  ...
  if( compute_control_using_MILP(weights, biases, glucose_inputs,
    insulin_inputs, model_prediction_horizon_steps, no_of_MPC_steps,
    physiological_constant, control_actions, target_val, total_insulin_constraint))
    {
      return 1;
    }
    else
...
int compute_control_action(
  vector< network_handler > prediction_networks,
  vector< datatype > glucose_inputs,
  vector< datatype > insulin_inputs,
  unsigned int no_of_MPC_steps,
  unsigned int model_prediction_horizon_steps,
...
{
  if(glucose_inputs.back() < 90)
  {
    control_actions.clear();
    control_actions.resize(glucose_inputs.size());
    fill(control_actions.begin(), control_actions.end(),  0.0);
    return 1;
  }
  unsigned int no_of_glucose_inputs = glucose_inputs.size();
  unsigned int no_of_insulin_inputs = insulin_inputs.size();

  if(prediction_networks.size() != 3)
  {
    cout << "Number of networks received does not match the expected number of networks !" << endl;
    cout << "Only " << prediction_networks.size() << " networks were received, exiting...  " << endl;
    exit(0);
  }
  ...
  prediction_networks[0].return_network_information(prediction_weights, prediction_biases);
  prediction_networks[1].return_network_information(upper_bound_weights, upper_bound_biases);
  prediction_networks[2].return_network_information(lower_bound_weights, lower_bound_biases);

  if( compute_control_using_MILP(prediction_weights, prediction_biases,
                             upper_bound_weights, upper_bound_biases,
                             lower_bound_weights, lower_bound_biases,
                             glucose_inputs, insulin_inputs, model_prediction_horizon_steps, no_of_MPC_steps,
                             physiological_constant, control_actions, target_val, total_insulin_constraint,
                             1.0 , false) )
  {
    // cout << "Reaches  here 0 " << endl;
    return 1;
  }
  else if (compute_control_using_MILP(prediction_weights, prediction_biases,
                             upper_bound_weights, upper_bound_biases,
                             lower_bound_weights, lower_bound_biases,
                             glucose_inputs, insulin_inputs, model_prediction_horizon_steps, no_of_MPC_steps,
                             physiological_constant, control_actions, target_val, total_insulin_constraint,
  ...
  {
    datatype impose_constraints_percent = 1.0;
    while(impose_constraints_percent > 0.0)
    {
      if( compute_control_using_MILP(prediction_weights, prediction_biases,
                                 upper_bound_weights, upper_bound_biases,
                                 lower_bound_weights, lower_bound_biases,
                                 glucose_inputs, insulin_inputs, model_prediction_horizon_steps, no_of_MPC_steps,
                                 physiological_constant, control_actions, target_val, total_insulin_constraint,
                                 impose_constraints_percent, true) )
      {
        // cout << ".................................... Reaches here 2 " << endl;
        // cout << "....................................  Constraints degree = " << impose_constraints_percent << endl;
        return 1;
      }
      impose_constraints_percent -= 0.1;
    }
  }
...
)
{
  ...
  {
    return_val = floor( (1.0/granularity) * input_value );
    return_val *= granularity;
  }
